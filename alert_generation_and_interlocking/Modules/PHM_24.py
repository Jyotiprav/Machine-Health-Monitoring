...
class PHM_24:
    def __init__(self,press_number):
        self.start_time = ""
        self.end_time = ""
        self.db_IP_address                  = "128.121.34.11"
        self.broker_IP_address              = "128.121.34.95"
        self.myclient_global                = pymongo.MongoClient(self.db_IP_address) 
        self.press_number                   = press_number
        self.datapoint_per_collection_dict  = {}
        self.component_list                 = []
        ...
        self.alert_collection               = self.alert_db[f"Press_{press_number}"] # Using testing database
    ...
    def set_current_job_and_ma_thresholds(self):
        try:
            ...
            print(recent_document_from_db[0])
    ...
    #=========================================================
    def get_temperature_rate_of_change(self,STATIC_rate_of_change_degrees=5,STATIC_temp_threshold=102):
        '''
            Description : 
            Function to get the difference between start time 
            temperature and end time temperature.
        '''
        sensors_per_batch_dict = self.get_current_datapoints_per_type("tmp")
        ...
        for batch,datapoints in sensors_per_batch_dict.items():
            ...
            for value in temperature_alert_dict.values():
                if value[1]>STATIC_temp_threshold:
                    # pass
                    status =self.send_critical_alert_to_PLC("128.121.34.95")
                    print(status)
...
press_11_object = PHM_24(press_number="24")

for i in range(6,7):   
    start_time  = datetime(2024,2,28,i-1,0,0)        #For temp testing
    end_time    = datetime(2024,2,28,i,0,0)          #For temp testing
    ...
    # print(press_11_object.all_datapoint_per_collection_dict)
    ...
    #         "Broker_Connection":True,
    #         "Database Connection":True,
    #         "Missing data":["sensor_name","missing"],
    #         "Constant data check":["sensor_name","Constant data"],
    #         "Moving average difference":{"R_L_Pin_Vrms":{"Rate of change":-59,
    #                                                      "Threshold mean value":0.004,
    #                                                      "Recent mean value":0.001}},
    #         "Number of peaks":{"Cr_LBlock1_Pre":{"Average peaks":{"340-90":34,"90-180":78,"180-270":78,"270-340":90},
    #                                              "Average pressure":{"340-90":34,"90-180":78,"180-270":78,"270-340":90}
    #                                              }}
    #     }
    ...
    #=========================================================
    # Get Anomalies using reconstruction error
    #=========================================================
    # def predict(self,model, test_data):
    #     # Use LSTM Autoencoder model to predict on test data.
    #     predictions = model.predict(test_data)
    #     return predictions
    # def calculate_error(self,test_data, predictions):
    #     """Calculate mean squared error and mean abs error between test data and predictions."""
    #     print("test_data shape", test_data.shape)
    #     print("predictions shape", predictions.shape)
    #     mse = np.mean(np.square(test_data - predictions), axis=1)
    #     mae = np.mean(np.abs(test_data - predictions), axis=1)
    #     return [mse,mae]
    # def get_reconstruction_error(self,datapoints_list = ["hyd_unitMain_psi"]):
    #     model_path      = r"D:\NGen Project files\3_Predictive Maintenance\github repos\Alert_generation_and_interlocking\Models\Press 24\hyd_unitMain_psi.keras"
    #     scaler_path     = r"D:\NGen Project files\3_Predictive Maintenance\github repos\Alert_generation_and_interlocking\Models\Press 24\hyd_unitMain_psi.pkl"
    #     model           = keras.models.load_model(model_path) # load the model
    #     start           = self.start_time
    #     end             = self.end_time
    #     ob              = dp.data_pre_processing()
    #     anomaly_limit   = 4 # For hyd_unitMain_psi , Mean squared error
        
    #     #Get the total number of documents
    #     total_documents = self.get_document_count(self.start_time,self.end_time)
        
    #     # Only when we have data in database, preprocess the data
    #     if total_documents!=None:
    #         print(f"get_reconstruction_error() : {total_documents} documents in database starting {self.start_time}.")
    #         # Get the number of samples in one second
    #         second_start = self.start_time
    #         second_end = second_start + timedelta(seconds=1)
    #         # n_steps = self.get_document_count(second_start,second_end) # Issue: Model is trained on 19 samples.
    #         n_steps = 19
    #         ob.set_values(start,end,n_steps,datapoints_list)
    #         X,df = ob.mongodb_to_X()
    #         # load the saved scaler
    #         with open(f"Modules/{datapoints_list[0]}.pkl", "rb") as infile:
    #             scaler = pkl.load(infile)
    #             data = scaler.transform(X.reshape(-1, X.shape[-1])).reshape(X.shape)
    #         trainPredict    = self.predict(model,data)
    #         trainMSE        = self.calculate_error(data, trainPredict)
    #         if trainMSE[0].max()>anomaly_limit: # MSE = 4 is the limit for anomalies.
    #             print('='*100)
    #             print("Anomalies encountered",trainMSE[0].max())
    #             print('='*100)
    #         else:
    #             print("No Anomalies Score was.",trainMSE[0].max())
...
# for i in range(0,23):   
#     try:
#         start_time  = datetime(2024,2,12,i,0,0)         #For temp testing
#         end_time    = datetime(2024,2,12,i+1,45,0)      #For temp testing

#         # start_time      = datetime(2024,1,31,11,0,0) 
#         # end_time        = datetime(2024,1,31,12,0,0)
        
#         press_object = PHM_24("24",start_time,end_time)
#         # print(press_object.get_temperature_rate_of_change("24")) # Testing temperature rate of change function on press 24.
#         press_object.get_reconstruction_error()
#         # press_object.generate_alert_code()
#         time.sleep(7)
#     except:
#         print("error")
